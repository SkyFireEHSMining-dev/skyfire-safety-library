...
function calculateSafetyRoi() {
  ...
  const projectedIncidentLoss =
    getNumberInput("roiPostIncidentEvents") *
    getNumberInput("roiCostPerIncident");

  const projectedNearMissLoss =
    getNumberInput("roiPostNearMissEvents") *
    getNumberInput("roiCostPerNearMissEvent");

  const projectedPropertyDamageLoss =
    getNumberInput("roiPostPropertyDamageEvents") *
    getNumberInput("roiCostPerPropertyDamageEvent");

  const projectedDowntimeLoss =
    getNumberInput("roiPostDowntimeEvents") *
    getNumberInput("roiDowntimeHoursPerEvent") *
    getNumberInput("roiCostPerDowntimeHour");

  const projectedAdminBurdenLoss =
    getNumberInput("roiPostAdminHoursPerYear") *
    getNumberInput("roiAdminHourlyRate");

  const projectedAnnualLossesAfterControl =
    projectedIncidentLoss +
    projectedNearMissLoss +
    projectedPropertyDamageLoss +
    projectedDowntimeLoss +
    projectedAdminBurdenLoss;

  const grossAnnualSavings =
    baselineAnnualLosses - projectedAnnualLossesAfterControl;

  const netAnnualSavings =
    grossAnnualSavings - annualRecurringCost;

  const calculatedReductionPercent =
    baselineAnnualLosses > 0
      ? (grossAnnualSavings / baselineAnnualLosses) * 100
      : 0;
  ...
  setText("roiProjectedAnnualLosses", formatCurrency(projectedAnnualLossesAfterControl));
  setText("roiCalculatedReductionPercent", formatPercent(calculatedReductionPercent));
...
function parseLegacyHeadParagraphXml(xmlText, config) {
  ...
  while ((match = regex.exec(xmlText)) !== null) {
    if (match[1]) {
      ...
      if (level === "title") {
        current = createBaseContext(config);
...
function parseCfrDocXml(xmlText, config) {
  ...
  const sectionNodes = Array.from(xmlDoc.querySelectorAll("SECTION"));

  sectionNodes.forEach(sectionNode => {
    const sectnoNode = sectionNode.querySelector(":scope > SECTNO");
    if (!sectnoNode) return;

    const sectnoText = normalizeHeadingSpacing(sectnoNode.textContent);
    if (!sectnoText) return;

    const subjectNode = sectionNode.querySelector(":scope > SUBJECT");
    ...
    const context = createBaseContext(config);

    while (parent) {
      const tag = parent.tagName ? parent.tagName.toUpperCase() : "";

      if (tag === "PART") {
        const hd = parent.querySelector(":scope > HD");
        if (hd) context.part = normalizeHeadingSpacing(hd.textContent);
      }

      if (tag === "SUBPART") {
        const hd = parent.querySelector(":scope > HD");
        if (hd) context.subpart = normalizeHeadingSpacing(hd.textContent);
      }

      if (tag === "SUBCHAP") {
        const hd = parent.querySelector(":scope > HD");
        if (hd) context.subchapter = normalizeHeadingSpacing(hd.textContent);
      }

      if (tag === "CHAPTER") {
        const hd = parent.querySelector(":scope > HD");
        if (hd) context.chapter = normalizeHeadingSpacing(hd.textContent);
      }

      if (tag === "TITLE") {
        const hd = parent.querySelector(":scope > HD, :scope > TITLENUM");
        if (hd) context.title = normalizeHeadingSpacing(hd.textContent);
      }

      parent = parent.parentElement;
    }

    const paragraphNodes = Array.from(sectionNode.querySelectorAll(":scope > P, :scope > FP, :scope > EXTRACT > P"));
...
function buildSectionsFromXmlText(xmlText, config) {
  ...
  if (config.key === "osha" || looksLikeCfrDoc) {
    ...
    if (cfrDocSections.length) return cfrDocSections;
  }
...
function buildHierarchy(sections, config) {
  ...
  sections.forEach(section => {
    ...
    if (!cursor.__sections) cursor.__sections = [];
...
function renderTree(libraryKey, node, levelIndex, query) {
  ...
  if (levelIndex >= LEVELS.length) {
    ...
    sortSectionsForDisplay(sections).forEach(section => {
      fragment.appendChild(createSectionBlock(libraryKey, section, query));
    });
    return fragment;
  }
...
async function runSearch(libraryKey) {
  ...
  if (!query) {
    status.textContent = `Showing all ${state.config.label} sections.`;
  ...
  sortSectionsForDisplay(matches).forEach(section => {
    wrapper.appendChild(createSectionBlock(libraryKey, section, query, {
