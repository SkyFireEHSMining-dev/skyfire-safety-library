(function(){
  ...
  const POSTER_ASSET="./assets/SkyFire-Part-50-Decision-Tree-approved.jpeg";
  ...
  function stylesheet(){if(document.querySelector('link[data-incident-ready-v16="true"]'))return;const l=document.createElement("link");l.rel="stylesheet";l.href="./incident-ready-v16.css?v=v0.16-incident-ready-3";l.dataset.incidentReadyV16="true";document.head.appendChild(l);}
  ...
  function stageLinks(s){
    const links={
      classify:[["cfr","50.2","Open 30 CFR §50.2"]],
      notify:[["cfr","50.10","Open 30 CFR §50.10"],["mine","§103","Open Mine Act §103"]],
      preserve:[["cfr","50.12","Open 30 CFR §50.12"]],
      investigate:[["cfr","50.11","Open 30 CFR §50.11"],["mine","§103","Open Mine Act §103"]],
      "7000":[["cfr","50.20","Open 30 CFR §50.20"],["form","","Open Form 7000-1"]],
      injury:[["cfr","50.2","Open 30 CFR §50.2"],["cfr","50.20-3","Open §50.20-3 first-aid criteria"]]
    }[s.id]||[];
    if(!links.length)return "";
    return '<div class="incident-stage-links" aria-label="Quick source links">'+links.map(link=>{
      if(link[0]==="form")return '<a href="'+FORM7000_URL+'" target="_blank" rel="noopener">'+esc(link[2])+'</a>';
      if(link[0]==="mine")return '<button type="button" data-open-mine103>'+esc(link[2])+'</button>';
      return '<button type="button" data-cfr="'+esc(link[1])+'">'+esc(link[2])+'</button>';
    }).join("")+'</div>';
  }
  function stageMarkup(s,i){return '<details class="incident-stage" id="incident-stage-'+esc(s.id)+'"><summary><strong>'+(i+1)+'. '+esc(s.title)+'</strong><span>'+esc(s.short)+'</span></summary><div class="incident-stage-body"><h4>Quick View / Do Now</h4><ul>'+s.quick.map(x=>'<li>'+esc(x)+'</li>').join("")+'</ul><h4>What this is</h4><p>'+esc(s.what)+'</p><h4>MSHA / regulatory lens</h4><p>'+esc(s.lens)+'</p><div class="incident-skyfire-guidance"><strong>SkyFire guidance:</strong> '+esc(s.guidance)+'</div>'+stageLinks(s)+'<div class="incident-source-row"><span class="incident-chip law">'+esc(s.authority)+'</span><span class="incident-chip gray">Source checked '+SOURCE_CHECKED+'</span></div></div></details>';}
  ...
  function markup(){
    return '<div class="module-header"><div class="module-header-text"><h2>Incident Ready — Part 50</h2><p>Source-backed incident decision support for immediate notification, scene preservation, investigation, and Form 7000-1.</p></div></div>'+
    ...
    '<div class="incident-visual-card" id="incidentPosterPreview"><div class="incident-kicker">Printable field companion</div><h3>MSHA Part 50 Decision Tree</h3><p>A whole-picture visual for supervisors and field use. The interactive workflow below remains the primary in-app decision tool.</p><div class="incident-visual-preview"><img loading="lazy" src="'+POSTER_ASSET+'" alt="Approved SkyFire MSHA Part 50 Decision Tree preview"></div><div class="incident-action-row"><button type="button" class="incident-action-btn field" data-open-poster>Open / Print Decision Tree</button><button type="button" class="incident-action-btn field" data-open-field-resource>View in Field Resources</button></div></div>'+
  ...
  function addSafetyDoc(){
    ...
    card.innerHTML='<h4>SkyFire MSHA Part 50 Decision Tree</h4><p>Printable whole-picture guide for immediate notification, scene preservation, operator investigation, injury/illness reporting, and Form 7000-1 decisions.</p><div class="part50-doc-preview"><img loading="lazy" src="'+POSTER_ASSET+'" alt="Approved SkyFire MSHA Part 50 Decision Tree field resource preview"></div><div class="document-actions"><button type="button" data-open-poster>Open / Print Visual</button><button type="button" data-open-incident-from-doc>Open Incident Ready</button></div><p class="part50-doc-meta">Source checked '+SOURCE_CHECKED+' · offline-cached field reference.</p>';
    ...
    card.querySelector("[data-open-poster]")?.addEventListener("click",openPoster);
    return true;
  }
  ...
  function initialize(){
    ...
    section.querySelectorAll("[data-open-mine103]").forEach(b=>b.addEventListener("click",()=>openMineAct("§103")));
    ...
    section.querySelectorAll("[data-open-poster]").forEach(b=>b.addEventListener("click",openPoster));
